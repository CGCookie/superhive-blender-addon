...
def setup_scene_collection(context: Context, col: bpy.types.Collection, shading="Material", add_plane=True):
    ...
    unselected: list[Object] = [
        ...
        if a.name not in selected_objects and a.type != "LIGHT"
    ]
    ...
    # // Put in function to share with single_id // #
def _capture_thumbnail_for_collection(context: Context, col: bpy.types.Collection, path: Path, shading="Material", angle='X', add_plane=True):
...
def setup_scene_single_id(context: bpy.types.Context, object: bpy.types.Object, shading="Material", add_plane=True) -> Path:
    scene: Scene = context.scene
    ...
    setup_engine(context, shading, scene, view_area)
    # // Put in function to share with single_id // #
def _capture_thumbnail(context: bpy.types.Context, object: bpy.types.Object, path: Path, shading="Material", add_plane=True) -> Path:
    with store_and_restore_scene_parameters(add_plane, context.scene) as (view_area, camera_object, track_object, ground_plane):
        view_area: Area
        camera_object: Object
        track_object: Object
...
def apply_thumbnail():
    ...
    for i, id_type in enumerate(TYPES):
        ...
        if id_type == 'COLLECTION':
            ...
            if ob:
                if ob.type in {'MESH','CURVE','SURFACE','TEXT','META'} and does_support_thumbnails(ob):
                    with context.temp_override(id=ob):
                        bpy.ops.ed.lib_id_load_custom_preview(filepath=str(thumbnail_path))
                        print("  - Thumbnail set")
                else:
    ...
    try:
        ...
        for i,id_type in enumerate(TYPES):
            ...
            if id_type == 'COLLECTION':
                ...
                ob = bpy.data.materials.get(id_name)
                objects.append(ob)
                if ob:
                    ob.asset_generate_preview()
            else:
