        ...
        with bpy.types.BlendData.temp_data() as bpy_data:
            ...
            for asset in assets:
                ...
                if data := getattr(data_to, data_type):
                    ...
                    bpy_data.libraries.write(str(dst_path), set(data), compress=True)
                    if self.pack_file:
    ...
    def add_to_library_keep(self, assets: list[AssetRepresentation], dir: Path):
        ...
        for blend_file, assets_in_file in blend_files.items():
            for asset in assets_in_file:
                src = Path(blend_file)
                dst = dir / src.with_stem(f"{src.stem}_{asset.name}").name
                utils.move_blend_file(src, dst)

                utils.clean_blend_file(
                    str(dst),
                    ids_to_keep=[asset.name],
...
class SH_OT_RemoveFromLibrary(Operator):
    ...
    bl_description = "Remove the selected asset(s) from their libraries. Hold Alt to remove the asset(s) from the blend file(s) as well"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def invoke(self, context, event):
        ...
        self.remove_from_blend = event.alt
        ...
        for lib, dct in self.asset_data.items():
            for blend_path, assets in dct.items():
                utils.clean_blend_file(
                    ...
                    remove=self.remove_from_blend,
                )

                with bpy.data.libraries.load(str(blend_path), assets_only=True) as (
                    data_from,
                    _,
                ):
                    has_attrs = False
...
class AddAsAsset(scene.RenderThumbnailProps):
    ...
    def assets_to_library(self, context: Context):
        ...
        self._thread = Thread(target=self.assets_to_library_thread, args=(context,))
    ...
    def finish(self, context: Context):
        ...
        if self.library == "NEW":
            for area in context.screen.areas:
                if polls.is_asset_browser(context):
                    area.spaces.active.params.asset_library_reference = (
                        self.new_library_name
                    )
        bpy.app.timers.register(self.follow_up, first_interval=1)
        return {"FINISHED"}
